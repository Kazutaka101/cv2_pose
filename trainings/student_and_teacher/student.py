...
data = {
    ...
    'time': str(dt_now),
    'system_info': str(system),
    'ip': str(ip.text),
    'training_mode':str(showSelectedMode),
    'count': str(cnt)
}
...
import json
# json dumpでstring化、utf-8でエンコード、文字列を数字にする、そして公開鍵で暗号化。
jsonData = json.dumps(data)
cipherData = public_cipher.encrypt(jsonData.encode())

fileName = "Training_report"+str(dt_now) 
fileName = fileName.replace(' ', '')
with open(fileName, 'wb') as f:
    f.write(cipherData)

print("Exported your report")
print(fileName)
